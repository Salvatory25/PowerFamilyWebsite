...
<body class="flex flex-col min-h-screen bg-slate-50 text-slate-900 font-sans antialiased selection:bg-[#c89a3b] selection:text-[#0c1c34]">
    ...
    <div class="bg-[#0c1c34] text-slate-300 text-xs py-2 px-3 sm:px-4 border-b border-slate-800/80">
        <div class="w-full max-w-[1720px] mx-auto flex flex-col md:flex-row justify-between items-center gap-2">
            
            <!-- Left: Regional Badge & Tagline -->
            <div class="flex items-center gap-2 sm:gap-2.5 flex-wrap justify-center md:justify-start text-center md:text-left">
                <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-[#c89a3b]/20 text-[#dfb256] font-bold text-[11px] ring-1 ring-[#c89a3b]/40 shrink-0">
                ...
                <span class="text-slate-300 font-medium text-[11px] sm:text-xs">
                    {{ $siteTagline ?: __('app.brand_tagline') }}
                </span>
            </div>
            
            <!-- Right: Direct Phone & Language Switcher -->
            <div class="flex items-center gap-3 sm:gap-4 shrink-0 justify-center">
                <!-- Phone Direct Link -->
                <a href="tel:{{ $sitePhone ?? '+255742448965' }}" class="inline-flex items-center gap-1.5 text-slate-200 hover:text-[#dfb256] font-semibold text-[11px] sm:text-xs transition">
                    <svg class="w-3.5 h-3.5 text-[#dfb256] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                ...
                <!-- Language Switcher -->
                <div class="flex items-center gap-1 font-medium bg-slate-900/80 px-2 py-0.5 rounded-lg border border-slate-800/80 text-[11px]">
                    <span class="text-slate-400">Lang:</span>
                    <a href="{{ route('lang.switch', 'en') }}" class="px-1.5 py-0.5 rounded transition {{ app()->getLocale() === 'en' ? 'bg-[#c89a3b] text-[#0c1c34] font-black' : 'text-slate-400 hover:text-white' }}">EN</a>
                    <span class="text-slate-600">|</span>
                    <a href="{{ route('lang.switch', 'sw') }}" class="px-1.5 py-0.5 rounded transition {{ app()->getLocale() === 'sw' ? 'bg-[#c89a3b] text-[#0c1c34] font-black' : 'text-slate-400 hover:text-white' }}">SW</a>
                </div>
            </div>
        </div>
    </div>
