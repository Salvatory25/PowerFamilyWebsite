...
<div class="bg-white border-b border-gray-100 py-3.5">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-xs font-semibold text-gray-500 flex items-center space-x-2">
        ...
        <span>/</span>
        <a href="{{ route('vehicles.index') }}" class="hover:text-[#750D15]">Magari</a>
        <span>/</span>
        ...
        <div class="bg-white rounded-2xl p-6 sm:p-8 shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="space-y-2">
                <div class="flex items-center space-x-3">
                    ...
                    <span class="px-3 py-1 rounded-md text-xs font-bold bg-[#FEF9EC] text-[#A86807] border border-[#F0E3B0]">
                        {{ $vehicle->make }}
                    </span>
                </div>
            ...
            <div class="lg:col-span-2 space-y-8">
                    ...
                    @if($vehicle->images->count() > 0)
                        <div class="flex items-center space-x-3 overflow-x-auto pb-2">
                            <button 
                                ...
                                :class="activeImage === '{{ $vehicle->display_image }}' ? 'border-[#750D15]' : 'border-transparent opacity-70'"
                            >
                ...
                <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div class="p-4 rounded-xl bg-[#FDF5F6] border border-[#F9E4E7]">
                        ...
                        <span class="text-base font-extrabold text-[#750D15] mt-0.5 block">{{ $vehicle->year }}</span>
                    </div>
                    <div class="p-4 rounded-xl bg-[#FDF5F6] border border-[#F9E4E7]">
                        <span class="text-[11px] font-semibold text-gray-500 uppercase block">Transmission</span>
                        <span class="text-base font-extrabold text-[#750D15] mt-0.5 block">{{ $vehicle->transmission }}</span>
                    </div>
                    <div class="p-4 rounded-xl bg-[#FDF5F6] border border-[#F9E4E7]">
                        <span class="text-[11px] font-semibold text-gray-500 uppercase block">Aina ya Mafuta</span>
                        <span class="text-base font-extrabold text-[#750D15] mt-0.5 block">{{ $vehicle->fuel_type }}</span>
                    </div>
                    <div class="p-4 rounded-xl bg-[#FDF5F6] border border-[#F9E4E7]">
                        <span class="text-[11px] font-semibold text-gray-500 uppercase block">Mileage / Rangi</span>
                        <span class="text-base font-extrabold text-[#750D15] mt-0.5 block">{{ $vehicle->mileage ?? ($vehicle->color ?? 'Clean') }}</span>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl p-6 shadow-xl border border-gray-100 sticky top-28 space-y-6">
                    ...
                    <div class="border-t border-gray-100 pt-6 space-y-4">
                        ...
                        <form action="{{ route('enquiry.submit') }}" method="POST" class="space-y-3">
                            ...
                            <div>
                                <textarea name="message" rows="3" required class="w-full bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2.5 text-xs focus:ring-2 focus:ring-[#750D15] focus:outline-none resize-none">Habari, ninahitaji taarifa zaidi kuhusu gari hili [{{ $vehicle->vehicle_reference }} - {{ $vehicle->title }}].</textarea>
                            </div>
