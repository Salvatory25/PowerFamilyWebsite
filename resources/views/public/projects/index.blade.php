...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div class="mb-12 flex flex-col md:flex-row items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-xs">
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('projects.index') }}" class="px-4 py-2 rounded-xl text-xs font-bold transition {{ !request('type') ? 'bg-[#750D15] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                {{ $isSw ? 'Miradi Yote' : 'All Projects' }}
            </a>
            @foreach($projectTypes as $type)
                <a href="{{ route('projects.index', ['type' => $type]) }}" class="px-4 py-2 rounded-xl text-xs font-bold transition {{ request('type') == $type ? 'bg-[#750D15] text-white shadow-xs' : 'bg-slate-100 text-slate-700 hover:bg-slate-200' }}">
                    {{ $type }}
                </a>
            @endforeach
        </div>
        ...
        <form action="{{ route('projects.index') }}" method="GET" class="flex items-center gap-2 w-full md:w-auto">
            <input type="text" name="location" value="{{ request('location') }}" placeholder="{{ $isSw ? 'Tafuta kwa eneo (mf. Kisongo)...' : 'Search by area (e.g. Njiro)...' }}" class="bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2 text-xs text-slate-800 focus:ring-2 focus:ring-[#750D15] focus:bg-white w-full md:w-60">
            <button type="submit" class="px-4 py-2 rounded-xl bg-[#D48B16] text-[#280508] font-bold text-xs hover:bg-[#b5882e] transition">
                {{ $isSw ? 'Tafuta' : 'Filter' }}
            </button>
        </form>
    </div>
                ...
                <div class="bg-white rounded-3xl overflow-hidden border border-slate-200 shadow-xs hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
                    <div class="relative h-56 overflow-hidden">
                        ...
                        <div class="absolute top-3 left-3 flex flex-col gap-1.5">
                            <span class="px-2.5 py-1 rounded-lg bg-[#280508]/90 text-[#FAC955] text-[10px] font-extrabold uppercase backdrop-blur-md">
                        ...
                        <div class="absolute bottom-3 left-3 right-3 flex items-center justify-between text-xs text-white bg-black/65 backdrop-blur-md px-3 py-1.5 rounded-xl">
                            ...
                            <span class="font-bold text-[#FAC955] shrink-0">{{ $project->size_covered }}</span>
                        </div>
                    </div>

                    <div class="p-6 flex-grow flex flex-col justify-between space-y-4">
                        <div>
                            <h3 class="font-extrabold text-base text-[#750D15] line-clamp-2 group-hover:text-[#D48B16] transition">
                                {{ $project->name }}
                            </h3>
                        ...
                        <div class="pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
                            ...
                            <a href="{{ route('projects.show', $project->slug) }}" class="font-bold text-[#750D15] hover:text-[#D48B16] transition inline-flex items-center gap-1">
    ...
    @else
        <div class="p-12 bg-white rounded-3xl border border-slate-200 text-center space-y-3">
            ...
            <a href="{{ route('projects.index') }}" class="inline-flex px-4 py-2 rounded-xl bg-[#750D15] text-white text-xs font-bold">
